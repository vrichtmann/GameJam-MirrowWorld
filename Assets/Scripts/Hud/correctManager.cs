...
public class correctManager : MonoBehaviour
{
    ...
    private IEnumerator hideHud(float waitTime)
    {
        ...
        StartCoroutine(openTheGate(0.6f));
        openGate.gameObject.active = true;
    }

    private IEnumerator openTheGate(float waitTime){
        yield return new WaitForSeconds(waitTime);
        openGate.GetComponent<Animator>().SetBool("open", true);
    }
}
